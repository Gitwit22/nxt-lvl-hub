...
function toProgramStatus(status: string): SuiteProgram["status"] {
...
// Suppress unused warning — roleWeight is available for role-sorting utilities
void roleWeight;
...
export function OrgProvider({ children }: { children: React.ReactNode }) {
  ...
  const { authUserId, isAuthenticated, me } = useAuth();
  ...
  // Auto-select the first org the user belongs to once orgs are loaded
  useEffect(() => {
    if (isLoading || activeOrgId) return;
    const memberships = me?.orgMemberships ?? [];
    const firstMatch = organizations.find((org) =>
      memberships.some((m) => m.orgId === org.id && m.active),
    );
    if (firstMatch) setActiveOrgId(firstMatch.id);
  }, [isLoading, organizations, me, activeOrgId]);
    ...
    return catalogPrograms.map((program) => ({
      ...
      launchUrl:
        program.type === "external"
          ? toBrowserDestination(program.externalUrl || `/applications/${program.id}`)
          : program.internalRoute || `/applications/${program.id}`,
      status: toProgramStatus(program.status),
  ...
  const createOrganization = async (input: CreateOrganizationInput) => {
    ...
    const organization: Organization = {
      ...
      welcomeMessage: input.welcomeMessage?.trim() || `Welcome to ${input.name.trim()}.`,
      ...
      announcements: [
        {
          ...
          message: "Organization workspace configured from platform admin.",
  ...
  const updateUser = async (userId: string, updates: Partial<PortalUser>) => {
    ...
    const merged: PortalUser = { ...current, ...updates };
  ...
  const updateOrganization = async (orgId: string, updates: Partial<Organization>) => {
    ...
    const updated = normalizeOrganization(
      await updateOrganizationRecord(orgId, toOrganizationMutationInput(merged)),
    );
...
// getOrgBootstrap is still available from @/lib/api for anything that needs it
export { getOrgBootstrap };
